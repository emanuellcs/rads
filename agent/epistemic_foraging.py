...
from core.constants import ACTION1, ACTION2, ACTION3, ACTION6, RESET_ACTION

# Abstract action mapping based on typical ARC-AGI-3 environment assumptions
ACTION_RIGHT = ACTION1
ACTION_LEFT = ACTION2
ACTION_DOWN = ACTION3
ACTION_TOGGLE = ACTION6
ACTION_RESET = RESET_ACTION
...
class EpistemicForager:
    ...
    def execute_mvp_sequence(self, env_interface) -> List[Dict[str, Any]]:
        ...
        # Step 4: Boundary Probe. ACTION2 is the standard opposite of ACTION1 in
        # the public API convention. This avoids sending RESET or an invalid action.
        _probe(ACTION_LEFT)
    ...
    def deliberate_hazard_probe(
        self, env_interface, suspected_hazard_action: int
    ) -> bool:
        """
    ...
    def check_hpc_condition(
        ...
        if num_active == 0:
            raise ValueError(
                "All world models falsified. Diffusion Prior must re-sample."
            )
...
def calculate_expected_information_gain(
    hypotheses: List[Any], candidate_action: int
) -> float:
    """
